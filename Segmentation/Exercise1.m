...
Image_nf = imfilter(Image_n, gauss, 'symmetric'); % Filtered version of "Image_n"
...
posClass = find(Image_nf_km == 2);
Image_PostProcess = zeros(size(Image_nf_km));

for i=1:length(posClass)
 Image_PostProcess(posClass(i)) = 1;
end

% figure(4)
% imagesc(Image_PostProcess)
% 
% pause(1)

%Post processing 

Image_PostProcess = bwmorph(Image_PostProcess, 'close', 50);
