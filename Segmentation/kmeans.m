...
while any(m-m0)
    ...
    for i = 1:length(I)
        %We search for the value of the cluster that is closest to the
        %value of the pixel
        %For this we look for the minimum of the substraction
        %Then we assign to the cluster list the position of that cluster
        a = abs(I(i)- m);
        [~, pos] = min(a); % For every pixel in the image
        c(i) = pos;        % Assign the label c(i)
    end
    ...
    %% STEP 2: Update step
    for k = 1:K 
        % For every class, compute the mean intensity in this class
        positions = find(c == k);
        %Compute the average
        avg = sum(I(positions))/length(positions);
        m(k) = avg;  
    end
