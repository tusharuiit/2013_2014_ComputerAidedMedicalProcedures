...
%TODO CHECK WHETHER IT IS ENOUGH OR NOT 
% [If I] = max(AllScale, [], 3);
If = I;
[row,col] = size(I);
for i=1:row
    for j=1:col
        If(i,j) = max(AllScale(i,j,:));
    end
end
...
Psignal = sum(sum((If-mean(mean(If))).^2))/numel(If);
noise = I - If ;
Pnoise = sum(sum((noise-mean(mean(noise))).^2))/numel(noise);
